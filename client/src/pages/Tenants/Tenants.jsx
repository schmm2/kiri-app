...
export default function Tenants() {
  ...
  const {
    loading: loadingMsGraphResource,
    error: errorMsGraphResource,
    data: dataMsGraphResource,
  } = useQuery(GetMsGraphResourcesDocument, {
    fetchPolicy: "cache-and-network",
    onCompleted: (dataMsGraphResource) => {
      console.log(dataMsGraphResource);
    },
  });
  ...
  const msGraphResourceMenu = (tenantDbId) => (
    <Menu>
      <Menu.Item key={"all"} onClick={() => triggerTenantUpdate(tenantDbId)}>
        All Resources
      </Menu.Item>
      {dataMsGraphResource &&
        dataMsGraphResource.msGraphResourceMany &&
        dataMsGraphResource.msGraphResourceMany.map(
          (msGraphResource, index) => {
            return (
              <Menu.Item
                key={msGraphResource.id}
                onClick={() =>
                  triggerTenantUpdate(tenantDbId, msGraphResource.id)
                }
              >
  ...
  async function triggerTenantUpdate(
    tenantDbId,
    msGraphResourceDbIdFilter = ""
  ) {
    ...
    postBackendApi("TRG2000ConfigurationBackupCreate", {
      ...
      .then((data) => {
        openNotificationWithIcon(
          "Backup",
          "Backup created successfully, start download",
          "success"
        );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Space size="middle">
          <a href="#" rel="noreferrer" onClick={() => triggerBackup(record.id)}>
            Backup
          </a>
          <a
            rel={"external"}
          ...
          <a
            href="#"
            rel="noreferrer"
            onClick={() => triggerDataCheck(record.id)}
          >
            Check Data
          </a>
          <a href="#" rel="noreferrer" onClick={() => triggerTest(record.id)}>
            Test
          </a>
          <a
            href="#"
            ...
            onClick={() => {
              deleteTenant({
                ...
                refetchQueries: [{ query: GetTenantsDocument }],
              });
            }}
          >
            Delete
  ...
  function addToDeployment(data) {
    ...
    let tenantIdsToAddToDeployment = selectedTenants.filter(
      (id) => deploymentTenantIds.indexOf(id) === -1
    );
    ...
    if (tenantIdsToAddToDeployment.length > 0) {
      ...
      openNotificationWithIcon(
        "Add to Deployment",
        "no action taken, already assigned",
        "success"
      );
    }
  }
  ...
  return (
    <div className="defaultPage">
      ...
      <div className="controlTop">
        <Space align="end">
          <Button disabled={selectedRows.length === 0} onClick={openModal}>
            + Deployment
          </Button>
        </Space>
      </div>
